...
/* ------------------------------------------------------------
   ...
   Chaque diapositive y renvoie via son champ `category` (ou aucun :
   `sujet` ouvre le parcours, `motifs` est la charnière entre la
   moitié classique et la moitié « apprentissage »). L'appartenance
   est une donnée : réaffecter une diapositive en changeant sa
   `category`. Le moteur ne lit que { label, color | colors } et reste générique.
   ------------------------------------------------------------ */
...
export const SLIDES = [
  /* ---------- ROOT ---------- */
 {
    ...
    seeds: {
      synthesis: `On peut décrire l'informatique de plusieurs façons, toutes équivalentes : transformer des représentations de l'information sous la contrainte de ressources finies ; ou gravir, couche après couche, une pile d'opérations qui va de l'impulsion électrique aux modèles génératifs, chaque niveau s'utilisant sans qu'on maîtrise celui du dessous ; ou, plus sobrement, admettre que l'intelligence apparente n'est jamais qu'une affaire de composition, sans ingrédient secret. Trois regards, un même objet. Tout l'enjeu du parcours est de dissiper ce qui ressemble à de la magie : montrer comment une machine calcule, et comment des données se muent en apprentissage puis en raisonnement. Sans quoi l'on ne peut juger sereinement ce dont l'IA d'aujourd'hui est, ou non, capable.`,
    },
 },
  /* ---------- LA MACHINE ---------- */
 {
    ...
    seeds: {
      synthesis: `Tout en bas, des milliards de transistors jouent le rôle de commutateurs, sans aucune pièce mobile, chacun figé dans l'un de deux états. Deux états suffisent, car les bits se combinent : n bits couvrent 2^n configurations. L'essentiel est là : rien dans la machine n'est « une image » ni « un texte » ; il n'y a que des motifs de bits, et une convention qui leur donne un sens. Le sens ne loge pas dans les bits, mais dans la manière convenue de les lire. C'est déjà « la carte n'est pas le terrain », et la racine de tous les encodages qui suivront : d'abord des bits, puis des vecteurs, puis des représentations apprises.`,
    },
 },

{
    ...
    seeds: {
      synthesis: `Le processeur ne sait faire qu'une poignée d'opérations élémentaires : calculer, comparer, déplacer ou copier des données, sauter d'une instruction à l'autre. Aucune n'a rien d'« intelligent » ; toute la puissance naît de leur composition, en très grand nombre et à très haute cadence. Surtout, le calcul est déterministe — mêmes entrées, mêmes étapes, même résultat, toujours — et purement mécanique : on manipule des symboles selon des règles fixes. C'est le socle du parcours, et sa thèse déflationniste : l'intelligence n'est que l'empilement de parties triviales, sans ingrédient spécial. Retenons ce déterminisme et cette transparence, car la couche générative, tout en haut, les troquera précisément contre de la portée.`,
    },
    },
{
    id: "memoire",
    ...
    seeds: {
      synthesis: `Deux idées se nouent ici. D'abord, calculer, c'est transformer puis stocker : chaque résultat doit être gardé quelque part pour l'étape suivante, si bien que la mémoire n'est pas un à-côté mais la moitié du calcul. Ensuite, la mémoire se hiérarchise, du petit et rapide (registres et caches, tout près du cœur) au vaste et lent (mémoire vive, disque, réseau, plus loin). Jamais les deux ensemble : stocker davantage, c'est stocker plus loin, donc plus lentement. Voilà le premier compromis physique du parcours. Et déplacer une donnée coûte souvent plus cher que la calculer — d'où la localité, les caches, l'art de ne pas refaire ce qui l'a déjà été. L'échelle des modèles s'y prépare déjà.`,
    },
  },
    {
    id: "arbitre",
    ...
    seeds: {
      synthesis: `Processeur, mémoire, fichiers : tout cela est en quantité finie, et c'est le système d'exploitation qui les répartit entre des programmes rivaux. Il passe de l'un à l'autre des milliers de fois par seconde — d'où l'illusion que tout tourne en même temps, même sur quelques cœurs — et il les cloisonne, chacun dans sa mémoire, pour qu'un plantage n'emporte pas la machine entière. Si on le place après le processeur et la mémoire, c'est à dessein : on ne comprend l'arbitrage qu'une fois vues les ressources rares qu'il arbitre. Première illusion soigneusement entretenue sur des ressources rationnées — un motif qui reviendra lorsqu'un même modèle figé servira d'innombrables usages. Et sous forte charge, l'illusion se fissure.`,
    },
  },

  {
    ...
    seeds: {
      synthesis: `Un langage de programmation laisse écrire des instructions en des termes proches du langage humain, qui nomment des intentions complexes ; un traducteur — compilateur ou interpréteur — les ramène, de proche en proche, aux minuscules opérations du processeur. Les langages de haut niveau sont plus expressifs et plus indulgents, mais éloignés de la machine ; ceux de bas niveau donnent plus de contrôle, au prix de plus de peine — un compromis de plus. Le point capital, que la section IA renversera : cette traduction est exacte, et vérifiable. La machine n'exécute jamais que de petites étapes ; le code n'en est que la surface, traduite fidèlement. Un compilateur est correct, sinon c'est un bug. Rien de neuf ne surgit dans la machine.`,
    },
  },
  {
    id: "algorithmes",
    ...
    seeds: {
      synthesis: `Un algorithme est une recette précise : une suite finie d'étapes qui résout un problème. Pour un même résultat, les recettes se distinguent surtout par leur coût, et ce qui compte, c'est la façon dont ce coût enfle avec la taille de l'entrée — c'est là que se joue le faisable. Quelques principes durables s'y rattachent : ne pas refaire ce qui l'a déjà été (mémoriser, réutiliser) ; mesurer avant d'optimiser, car on se trompe souvent sur le point lent ; se méfier de toute métrique érigée en objectif, qui finit détournée (Goodhart) — encore « la carte n'est pas le terrain ». Cette croissance du coût reviendra transfigurée à l'échelle des grands modèles : moteur, plutôt que menace.`,
    },
  },

  {
    ...
    seeds: {
      synthesis: `Devant la complexité, le geste maître est d'abstraire : découper une tâche difficile en tâches plus simples, puis enfermer chacune derrière une interface — un contrat qui dit ce qu'elle fait en taisant comment. Séparer le quoi du comment, c'est justement ce qui permet de composer librement des fonctions et de bâtir du complexe à partir du simple : tant que le quoi tient, le comment se change à volonté. Nommer et composer — nommer une capacité, puis combiner des capacités nommées — fait presque tout le métier de construire. C'est la face constructive du thème de la composition, et elle annonce le contraste le plus net avec l'IA : ces interfaces tiennent leur promesse exactement, là où une couche apprise ne la tient que probablement.`,
    },
  },
  {
    id: "assemblages",
    ...
    seeds: {
      synthesis: `Un logiciel moderne s'appuie sur une haute pile d'abstractions écrites par d'autres — pilotes, système d'exploitation, langages, bibliothèques, frameworks — chacune réputée fiable à travers son interface. On n'écrit presque rien de zéro ; on assemble des briques existantes. Mais la complexité cachée derrière chaque interface est repoussée, non effacée — jamais tout à fait « évacuée » — et les abstractions finissent par fuir. Qu'une bibliothèque flanche bizarrement, qu'une requête traîne sans raison, qu'une hypothèse tacite cède, et le « comment » scellé refait surface, obligeant à ouvrir ce qu'on n'a pas écrit. D'où la règle de l'artisan : se servir du sommet, mais comprendre un cran plus bas. Elle vaudra plus encore pour l'IA, couche probabiliste, donc la plus fuyante de toutes.`,
    },
  },
{
    id: "perturbations",
    ...
    seeds: {
      synthesis: `Assembler, c'est choisir, et choisir, c'est prévoir que le monde ne suivra pas le scénario idéal. Un système fiable est fait pour tenir debout quand les choses lâchent et bougent — ce qui arrive sans cesse : le réseau coupe, le code évolue et déplace ses interfaces, des sources de vérité divergent, une entrée extérieure se révèle non conforme ou hostile, répartir le travail entre machines crée collisions et conflits d'accès, la charge éprouve chaque ressource. La bonne ingénierie l'intègre d'emblée : valider les entrées, encaisser les pannes, ne jamais présumer d'une copie unique, protéger les accès partagés. Cette prudence est exactement celle que la seconde moitié du parcours exigera de l'IA — dérive, entrée douteuse, erreur qu'on assume.`,
    },
  },

   {
    ...
    fragment: `Gardons certaines idées à l'esprit — en manipulant des symboles`,
    ...
    seeds: {
      synthesis: `Certaines idées ne tiennent pas à un seul barreau : elles reviennent à tous. La première : comprendre un cran plus bas. On peut se servir du sommet sans le socle, mais cerner les limites d'une abstraction demande de penser sous son interface. La deuxième : la carte n'est pas le terrain. Délimiter un objet, c'est figer ce qu'il est et ce qu'il n'est pas, et couler un besoin réel dans des symboles en perd toujours quelque chose — une métrique n'est pas le but, un modèle n'est pas le monde, une spécification n'est pas le besoin, et c'est dans cet écart que se nichent les pires échecs. La troisième : presque tout est compromis — vitesse, taille, cohérence, disponibilité, simplicité, sûreté s'accordent rarement —, autant l'arbitrer en connaissance de cause.`,
    },
  },
{
    id: "apprendre",
    ...
    seeds: {
      synthesis: `Jusqu'ici, un humain posait des règles explicites qu'une machine déterministe suivait. Mais certaines tâches résistent — reconnaître un visage, filtrer un courriel indésirable, traduire — car nul ne sait en énumérer les règles : elles vivent dans des millions d'exemples, non dans une spécification. L'apprentissage automatique renverse alors le dispositif : au lieu d'écrire le comment, on montre des exemples du quoi — des entrées et les sorties voulues — et un algorithme cherche une règle qui les ajuste et, surtout, s'étend aux cas nouveaux. Compromis fondateur : on gagne en portée — des tâches trop variées ou trop mouvantes pour se spécifier à la main — et l'on paie en garanties et en lisibilité. Les règles deviennent statistiques et approchées ; les données, elles, deviennent le programme.`,
    },
  },

  {
    ...
    seeds: {
      synthesis: `Comme les machines excellent à manipuler des nombres, on représente chaque objet — courriel, visage, mot, client — par un vecteur, un point dans un espace de coordonnées, agencé pour que les objets proches par le sens le soient aussi par la distance, et les autres, éloignés. Dès que le sens devient distance, « trouver un semblable », « regrouper », « recommander » se ramènent à de la géométrie — comme si celle-ci pouvait porter le sens. C'est ici que la portée promise par le nouveau paradigme se concrétise vraiment : le mécanisme qui fait marcher l'apprentissage. On retrouve l'encodage déjà croisé au niveau des bits — le sens comme convention posée sur des nombres — mais appris, cette fois, et tourné vers la prédiction, plutôt que fixé par un humain.`,
    },
  },
   {
    id: "correlation",
    ...
    seeds: {
      synthesis: `Un modèle saisit des corrélations — ce qui va avec quoi — non des causes. Prédire n'est pas expliquer : voir deux grandeurs varier ensemble ne dit pas que l'une commande l'autre, ni qu'un troisième facteur, caché, ne les commande pas toutes deux. C'est « la carte n'est pas le terrain », version apprentissage : le modèle attrape l'ombre du monde, pas son mécanisme. Deux limites s'ajoutent. Les observations restent toujours parcellaires — un instantané, jamais le tout —, si bien que le motif s'appuie sur un fragment ; et le monde bouge, de sorte que ce qu'on a appris hier se périme en silence (la dérive), emportant avec lui les modèles censés l'expliquer. C'est la dérive annoncée plus tôt, désormais au cœur même du modèle.`,
    },
  },
  {
    id: "generalisation",
    ...
    seeds: {
      synthesis: `Apprendre n'est pas mémoriser : la réussite se juge sur des cas mis de côté, jamais vus pendant l'entraînement. Un modèle qui prédit bien sur des situations vraiment inédites a sans doute saisi une relation sous-jacente, plutôt que retenu des réponses — c'est cela, généraliser, et c'est le vrai but. Mais comme l'avenir réel échappe à tout test, on ne peut qu'estimer cette généralisation par un substitut — un jeu de test réservé — en acceptant une erreur résiduelle : le modèle se trompera sur certains cas, et l'honnêteté consiste à s'y préparer, non à le cacher. Deux motifs reparaissent : ce jeu de test n'est qu'une carte de la performance à venir, donc détournable comme toute métrique-objectif ; et prévoir l'erreur pour bâtir avec prolonge la prudence défensive.`,
    },
  },
 {
    id: "incertitude",
    ...
    seeds: {
      synthesis: `Une prédiction sans idée de sa propre fiabilité n'en est qu'à moitié une : elle pousse à faire autant confiance à une quasi-certitude qu'à un pari hasardeux. Une prédiction utilisable s'accompagne donc de la confiance qui l'a produite — et cette confiance doit elle-même être calibrée : sur tous les cas annoncés à 70 %, il faudrait tomber juste environ soixante-dix fois sur cent. Connaître l'incertitude change la décision : « il se pourrait qu'il pleuve » et « il va pleuvoir » n'appellent pas les mêmes choix ; on suit l'annonce sûre, on vérifie ou l'on se couvre pour l'annonce fragile. C'est la sortie constructive de tout le versant épistémique : à défaut de certitude, on peut au moins la mesurer et l'afficher.`,
    },
  },
{
    id: "motsuivant",
    ...
    seeds: {
      synthesis: `Un grand modèle de langage s'entraîne sur une tâche d'une simplicité déroutante : étant donné un texte, deviner le token suivant, sur d'immenses corpus. La formule qui dégonfle la magie — « il ne fait que prédire le mot suivant » — est juste. Mais la vérité complète tient les deux bouts : menée assez bien, sur à peu près tout — grammaire, sémantique, faits historiques, techniques, scientifiques —, cette prédiction fait surgir des connaissances, un style, et quelque chose qui ressemble à du raisonnement, comme autant d'effets de bord que personne n'a programmés. C'est le thème de la composition mené à son point le plus troublant. Conséquence décisive : les données d'entraînement deviennent une part du programme, et la qualité, de nouveau, ne se garantit que probabilistement.`,
    },
  },
  {
    id: "taille",
    ...
    seeds: {
      synthesis: `Ce tour de force de la prédiction du mot suivant n'opère qu'à très grande échelle : les modèles récents alignent des centaines de milliards de paramètres — des « boutons » — réglés pour mieux prédire sur des milliers de milliards d'exemples. Ce réglage demande une quantité vertigineuse d'arithmétique, que rendent possible le matériel — des GPU faits pour le calcul massivement parallèle — et le logiciel qui le répartit et l'optimise. Au fond, l'essor de l'IA prolonge la vieille histoire de la mémoire et de l'arithmétique, à une échelle inédite. Passé certains seuils, des capacités nouvelles semblent surgir de nulle part (l'émergence) — non par miracle, mais parce que la composition produit un comportement plus vite qu'on ne sait l'énoncer, ni même le voir.`,
    },
  },
{
    id: "couts",
    ...
    seeds: {
      synthesis: `L'économie des modèles tient à une forte asymétrie entre deux phases. L'entraînement est lourd, mais massivement parallélisable : un coût unique et considérable qu'on peut étaler sur quantité de processeurs à la fois ; on le paie une fois, puis l'on fige le résultat. L'usage — l'inférence, la génération — coûte bien moins par appel, mais reste séquentiel par nature : le modèle produit un token, puis le suivant, chacun suspendu au précédent, sans se paralléliser de même. La contrainte glisse alors du calcul brut vers la latence, le débit, la disponibilité : servir vite beaucoup d'usagers relève de l'ordonnancement et de la circulation des données. C'est l'arbitrage du système d'exploitation à un autre étage : une capacité générale figée, puis amortie sur d'innombrables usages.`,
    },
  },

  {
    ...
    fragment: `L'IA générative est la couche supérieure de la pile d'abstraction — une couche non déterministe`,
    ...
    seeds: {
      synthesis: `L'IA générative apparaît comme la couche la plus haute de la pile d'abstraction — mais une couche non déterministe. Toutes les précédentes traduisaient exactement, et de façon vérifiable, d'un niveau à l'autre ; celle-ci rend la traduction statistique : au lieu de composer strictement des opérations élémentaires, on pose un contexte et on laisse le modèle produire une réponse, tributaire des données d'entraînement comme de celles qu'on lui injecte. Marché à deux visages. On gagne en portée : plus riche que le code, la langue naturelle laisse tenter ce que nul ne saurait spécifier exactement. On perd en contrôle et en certitude : plus floue que le code, cette couche échoue de façon fluide et assurée. Elle ne promet plus d'avoir raison, seulement probablement — d'où l'indispensable vérification humaine.`,
    },
  },
{
    id: "agents",
    ...
    seeds: {
      synthesis: `Un agent est un système de raisonnement probabiliste auquel on donne les moyens de percevoir son environnement, d'agir et de recommencer — appeler des outils, décider, avancer pas à pas — au lieu de rendre une seule réponse ; il enchaîne ainsi des tâches complexes. C'est là que le « probablement juste » prend tout son poids : une réponse fausse, on la lit et on l'écarte, mais une action fausse, elle, s'exécute, et de menues erreurs se composent en conséquences qu'aucun pas n'avait voulues, d'une portée qui croît avec celle de la capacité. Tous les motifs du parcours y ressurgissent — dérive, abstractions qui fuient, écart entre le but affiché et le besoin réel. La couche la moins sûre reçoit le plus grand pouvoir d'agir : d'où l'étape de vérification.`,
    },
  },
];
